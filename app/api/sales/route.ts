...
export async function POST(request: Request) {
  try {
    ...
    const normalizedSaleNumber = typeof sale_number === 'string' ? sale_number.trim() : ''
    ...
    if (normalizedSaleNumber && normalizedSaleNumber.length > 60) {
      return NextResponse.json(
        {
          success: false,
          error: 'Invoice number must be 60 characters or fewer',
          code: 'VALIDATION_ERROR',
        },
        { status: 400 }
      )
    ...
    if (normalizedSaleNumber) {
      const { data: existingSale, error: existingSaleError } = await supabaseAdmin
        ...
        .eq('sale_number', normalizedSaleNumber)
        .limit(1)
      ...
      if (existingSale && existingSale.length > 0) {
        return NextResponse.json(
          {
            success: false,
            error: 'Invoice number already exists for this store',
            code: 'VALIDATION_ERROR',
          },
          { status: 400 }
        )
    ...
    const hasCustomSaleNumber = Boolean(normalizedSaleNumber)
    const saleNumber = hasCustomSaleNumber
    ...
    if (!hasCustomSaleNumber) {
      ...
      sale.sale_number = normalizedSaleNumber
    }
